...
class StyleManager extends React.Component {
	...
	state = {
		...
		boxShadowRep: 0,
		boxShadowValue:
			this.props.selected.node &&
			_grapesEditor.styleManager.getStyles(
				this.props.selected,
	...
	componentDidUpdate(prevProps, prevState) {
		...
		if (prevProps.selected != this.props.selected) {
			// this.forceUpdate()
			//console.log("from styleManager", this.props.styleObj);
			this.extractTransform();
		}
		if (
			this.props.selected &&
			prevProps.selected.node != this.props.selected.node
		) {
			this.setState(
				{
					boxShadowValue:
	...
	setcompositeHidden = (prevState) => {
		if (
			prevState.showBackgroundComposite != this.state.showBackgroundComposite
		) {
			document.querySelector(
				".background-composite-label .composite-open"
			).style.display = this.state.showBackgroundComposite ? "flex" : "none";
			document.querySelector(
				".background-composite-label .composite-closed"
			).style.display = !this.state.showBackgroundComposite ? "flex" : "none";
		}
		if (prevState.showBoxShadowComposite != this.state.showBoxShadowComposite) {
			document.querySelector(
				".box-shadow-composite-label .composite-open"
			).style.display = this.state.showBoxShadowComposite ? "flex" : "none";
			document.querySelector(
				".box-shadow-composite-label .composite-closed"
			).style.display = !this.state.showBoxShadowComposite ? "flex" : "none";
		}
		if (
			prevState.showTextShadowComposite != this.state.showTextShadowComposite
		) {
			document.querySelector(
				".text-shadow-composite-label .composite-open"
			).style.display = this.state.showTextShadowComposite ? "flex" : "none";
			document.querySelector(
				".text-shadow-composite-label .composite-closed"
			).style.display = !this.state.showTextShadowComposite ? "flex" : "none";
		}
	};
	setCompositeRep = () => {
		...
		this.handlechange(
			"backgroundRep",
			(value[0] && value[0].color !== "rgba(0, 0, 0, 0)") || value.length > 1
				? value.length
				: 0
		);
	...
	globalOnChange = (obj, formData) => {
		...
		requiredStyleObj.selector =
			pseudoClass == "normal"
				? "." + uniqueClass
				: `.${uniqueClass}:${pseudoClass}`; // if selector not there
		...
		let data = _grapesEditor.styleManager.extractBlock(
			`${requiredStyleObj.selector} {`,
			str
		);
		if (data.customCode != "") {
		...
		customEvents.saveStyleInfo(
			{ elem: selected.node, node: editorNode },
			...
			() => {
				...
				this.state.globalOnchangeCallBack &&
					this.state.globalOnchangeCallBack();
			}
		);
	};
			...
			return (
				<div key={key}>
					<div
						...
						{item.render ? item.render() : label(item, key)}
					</div>
					{opened == key && (
						<div className={item.childerClass || "category-children"}>
							{item.children}
						</div>
					)}
				</div>
	...
	extractTransform = () => {
		...
		let transform =
			(selected.styleInfo.styles && selected.styleInfo.styles[["transform"]]) ||
			"";
	...
	extractBackgroundProperty = () => {
		...
		_.each(backgroundImage, (image, key) => {
			if (image != "none") {
				let imageDetails = {
					image: image.trim(),
					blendMode: backgroundBlendMode[key]
						? backgroundBlendMode[key].trim()
						: backgroundBlendMode[0].trim(),
					repeat: backgroundRepeat[key]
						? backgroundRepeat[key].trim()
						: backgroundRepeat[0].trim(),
					position: backgroundPosition[key]
						? backgroundPosition[key].trim()
						: backgroundPosition[0].trim(),
					attachment: backgroundAttachment[key]
						? backgroundAttachment[key].trim()
						: backgroundAttachment[0].trim(),
					size: backgroundSize[key]
						? backgroundSize[key].trim()
						: backgroundSize[0].trim(),
	...
	updateBackgroundProperty = (valArr, cb = () => {}) => {
		...
		if (count.color > 1) {
			let lastIndex = _.findLastIndex(valArr, function (o) {
				return o.type == "color";
			});
	...
	backgroundCompositeField({
		...
		const { backgroundImage, assetsManager } = useSelector(({ editor }) => ({
		...
		let switcher = [
			{
				...
				render: (value) => {
					return (
						<div className={"background-type-shift-btn"}>
							<div
								className={backgroundKey == "image" ? "selected" : "inactive"}
								style={{
									borderRadius:
										backgroundKey == "image" ? "4px 0px 0px 4px" : "0px",
								}}
								onClick={() => {
									if (backgroundKey == "image") return;
									handleOnChange({
										key: "image",
										value: 'url("http://grapesjs.com/img/work-desk.jpg")',
										switcher: true,
									});
							...
							<div
								className={backgroundKey == "color" ? "selected" : "inactive"}
								onClick={() => {
							...
							<div
								className={
									backgroundKey == "gradient" ? "selected" : "inactive"
								}
								style={{
									borderRadius:
										backgroundKey == "gradient" ? "0px 4px 4px 0px" : "0px",
								}}
								onClick={() => {
									...
									handleOnChange({
										key: "gradient",
										value:
											"linear-gradient(90deg, rgba(2,0,36,1) 6%, rgba(1,87,126,1) 94%)",
										switcher: true,
									});
		...
		let blendMode = {
			...
			// width: '48%',
			options: [
				//optional type: Array of string, Array of objects
				{
					label: "Normal",
					value: "normal",
				},
		...
		let imageFields = [
			...
			},
			{
				...
				key: "size",
				type: "select", //required
				value: ["contain", "cover", "auto", "initial"].includes(
					value && value.size
				)
					? value.size
					: "initial",
		...
		return (
			<div className={"background-composite-field"}>
	...
	textShadowCompositeField({
		...
		useEffect(() => {
			if (value != null) {
				...
				let resp = `${extractValue(newValue[1])}px ${extractValue(
					newValue[2]
				)}px ${extractValue(newValue[3])}px ${newValue[0]}`;
		...
		let handleOnChange = (item, action = "") => {
			...
			if (val || action === "delete") {
				...
				let resp = `${extractValue(
					item.key == "hOffset" ? val : hOffset
				)}px ${extractValue(
					item.key == "vOffset" ? val : vOffset
				)}px ${extractValue(item.key == "blur" ? val : blur)}px ${
					item.key == "color" ? val : color
				}`;
	...
	boxShadowCompositeField({
		...
		useEffect(() => {
			if (value != null) {
				...
				let resp = `${extractValue(newValue[1])}px ${extractValue(
					newValue[2]
				)}px ${extractValue(newValue[3])}px  ${extractValue(newValue[4])}px ${
					newValue[0]
				} ${inside ? "inset" : " "}`;
				onChange(resp, key, "setArray");
			}
		...
		let handleOnChange = (item, action = "") => {
			...
			if (val || action === "delete") {
				...
				let resp = `${extractValue(
					...
					item.key == "vOffset" ? val : vOffset
				)}px ${extractValue(item.key == "blur" ? val : blur)}px ${extractValue(
					item.key == "spread" ? val : spread
				)}px ${item.key == "color" ? val : color} ${
		...
		resp = [
			{
				...
				render: () => {
					return (
						<div
							className={"typography-fonts-googlefonts"}
							onClick={() => {
								dispatch(openAssets({ type: "font" }));
							}}>
							Google Fonts
							<Icons.OpenLink
								style={{ width: "10px", height: "10px", marginLeft: "6px" }}
	...
	render() {
		...
		const generalFormFields = [
			{
				...
				value:
					selected.node &&
					_grapesEditor.styleManager.getStyles(selected, pseudoClass, "float"),
				// width: '48%',
				options: [
					//optional type: Array of string, Array of objects
					{
						label: (
							<Icons.Cross
								className={"rightArrow"}
								style={{ width: "8.25px", height: "8.25px" }}
							/>
						),
						value: "none",
					},
					{
						label: (
							<Icons.AlignLeft
								className={"rightArrow"}
								style={{ width: "13.33px", height: "7.5px" }}
							/>
						),
						value: "left",
					},
					{
						label: (
							<Icons.AlignRight
								className={"rightArrow"}
								style={{ width: "11.67px", height: "7.5px" }}
							/>
						),
						value: "right",
					},
			...
			},
			{
				...
				value:
					selected.node &&
					_grapesEditor.styleManager.getStyles(selected, pseudoClass, "top"),
				defaultUnit: "px",
				unit: ["px", "%"],
				width: "48%",
			},
			{
				...
				value:
					selected.node &&
					_grapesEditor.styleManager.getStyles(selected, pseudoClass, "right"),
				defaultUnit: "px",
				unit: ["px", "%"],
				width: "48%",
			},
			{
				...
				value:
					selected.node &&
					_grapesEditor.styleManager.getStyles(selected, pseudoClass, "left"),
				defaultUnit: "px",
				unit: ["px", "%"],
				width: "48%",
			},
			{
				...
				value:
					selected.node &&
					_grapesEditor.styleManager.getStyles(selected, pseudoClass, "bottom"),
				defaultUnit: "px",
				unit: ["px", "%"],
				width: "48%",
			},
		...
		const dimensionFormFields = [
			{
				...
				value:
					selected.node &&
					_grapesEditor.styleManager.getStyles(selected, pseudoClass, "width"),
				defaultUnit: "px",
				unit: ["px", "%"],
				width: "48%",
			},
			{
				...
				value:
					selected.node &&
					_grapesEditor.styleManager.getStyles(selected, pseudoClass, "height"),
				defaultUnit: "px",
				unit: ["px", "%"],
				width: "48%",
			},
			...
			},
			{
				...
				render: (value, globalOnChange) => {
					...
					let handleOnChange = (val, key) => {
						//val: 20px, key: 'margin.top'
						key = key.split(".");
						let evaluatedValue =
							key[0] == "margin" ? _.clone(margin) : _.clone(padding);
		...
		const typographyFormFields = [
			...
			},
			{
				...
				value:
					selected.node &&
					_grapesEditor.styleManager.getStyles(selected, pseudoClass, "color"),
			},
			{
				type: "custom",
				key: "line-height",
					...
					return (
						<div className={"letter-spacing"}>
							...
							<input
								...
								onChange={(e) => {
									(value != "" || value != null) &&
										globalOnChange(`${e.target.value}px`);
								}}
							/>
						</div>
					);
			...
			},
			{
				...
				value:
					selected.node &&
					_grapesEditor.styleManager
						.getStyles(selected, pseudoClass, "text-decoration")
						.split(" ")[0],
				width: "48%",
				options: [
					//optional type: Array of string, Array of objects
					...
					},
					{
						label: (
							<Icons.Strikethru
								className={"text-decoration-linethru"}
								style={{ width: "12px", height: "12px" }}
							/>
						),
						value: "line-through",
					...
					},
					{
						label: (
							<Icons.AlignJustified
								className={"text-align-justified"}
								style={{ width: "11.67px", height: "7.5px" }}
							/>
						),
						value: "justify",
			...
			},
			{
				label: () => {
					return (
						<div
							className={
								"composite-label custom-label text-shadow-composite-label"
							}>
							Text Shadow
							<div
								style={{
								...
								<Icons.Plus
									onClick={() => {
										this.setState({
											textShadowRep: this.state.textShadowRep + 1,
										});
									}}
									style={{ width: "12px", height: "12px" }}
								/>
				...
				children: (value, key, onChange) => {
					return (
						<this.textShadowCompositeField
							{...{
								value,
								position: key,
								onChange,
								global: this.globalOnChange,
								textShadowRep: this.state.textShadowRep,
								updateRep: this.handlechange,
							}}
						/>
					);
				},
		...
		const decorationsFormFields = [
				...
				render: (value, globalOnChange) => {
					...
					if (value) {
						if (value.split(" ").length > 1) {
							...
							integratedBorderRadius &&
								this.setState({ integratedBorderRadius: false });
						} else {
							//single
							...
							<div className={"border-radius-integer"}>
								<Integer
									meta={{
										defaultUnit: "%",
										value: value,
										disabled: !integratedBorderRadius,
									}}
									globalOnChange={(val) => {
										globalOnChange(`${val}`);
									}}
								...
								<div
									className={
										integratedBorderRadius
											? "independent-border-radius-btn"
											: "independent-border-radius-btn-selected"
									}
									onClick={() => {
										this.setState({
											integratedBorderRadius: !integratedBorderRadius,
										});
									}}>
									<Icons.BorderRadius
										style={{ width: "12px", height: "12px" }}
			...
			},
			{
				...
				render: (value, globalOnChange) => {
					...
					return (
						<div className={"border-radius-independent-container"}>
			...
			},
			{
				...
				options: [
					...
					{
						label: (
							<Icons.DashedLine style={{ width: "30px", height: "2px" }} />
						),
						value: "dashed",
					},
					{
						label: (
							<Icons.DottedLine style={{ width: "30px", height: "2px" }} />
						),
						value: "dotted",
					},
				],
			...
			},
			{
				label: () => {
					return (
						<div
							className={
								"composite-label custom-label box-shadow-composite-label"
							}>
							Box Shadow
							<div
								style={{
								...
								<Icons.Plus
									onClick={() => {
										this.setState({
											boxShadowRep: this.state.boxShadowRep + 1,
										});
									}}
									style={{ width: "12px", height: "12px" }}
								/>
				...
				children: (value, key, onChange) => {
					return (
						<this.boxShadowCompositeField
							{...{
								value,
								position: key,
								onChange,
								global: this.globalOnChange,
								boxShadowRep: this.state.boxShadowRep,
								updateRep: this.handlechange,
							}}
						/>
					);
			...
			},
			{
				label: () => {
					return (
						<div
							className={
								"composite-label custom-label background-composite-label"
							}>
							Background
							<div
								style={{
								...
								<Icons.Plus
									onClick={() => {
										...
										this.setState({
											backgroundRep: this.state.backgroundRep + 1,
											backgroundValue: backgroundArr,
										});
									}}
									style={{ width: "12px", height: "12px" }}
								/>
				...
				children: (value, key, onChange) => {
					return (
						<this.backgroundCompositeField
							{...{
								value,
								position: key,
								backgroundRep: this.state.backgroundRep,
								updateRep: this.handlechange,
								onChange,
		...
		const extraFormFields = [
			...
			},
			{
				...
				render: (value, globalOnChange) => {
					const { transformKey } = this.state;
					return (
						<div className={"rotate-scale-shift-btn"}>
							<div
								className={transformKey == "rotate" ? "selected" : "inactive"}
								onClick={() => {
									this.handlechange("transformKey", "rotate");
								}}>
								Rotate
							</div>
							<div
								className={transformKey == "scale" ? "selected" : "inactive"}
								onClick={() => {
									this.handlechange("transformKey", "scale");
								}}>
								Scale
							</div>
						</div>
		...
		const flexFormFields = [
			...
			},
			{
				...
				value:
					selected.node &&
					_grapesEditor.styleManager.getStyles(selected, pseudoClass, "order"),
				// defaultUnit: 'px',
				// unit: ['px', '%'],
				width: "48%",
			},
			{
				...
				options: [
					...
					{
						label: (
							<Icons.FDirRowRev style={{ width: "20px", height: "20px" }} />
						),
						value: "row-reverse",
					},
					{
					...
					{
						label: (
							<Icons.FDirColRev style={{ width: "20px", height: "20px" }} />
						),
			...
			},
			{
				...
				options: [
					...
					{
						label: (
							<Icons.JustifyStart style={{ width: "6.45px", height: "15px" }} />
						),
						value: "start",
					},
					{
						label: (
							<Icons.JustifyEnd style={{ width: "6.45px", height: "15px" }} />
						),
						value: "end",
					},
					{
						label: (
							<Icons.JustifySpcBtw
								style={{ width: "12.86px", height: "15px" }}
							/>
						),
						value: "space-between",
					},
					{
						label: (
							<Icons.JustifySpcArnd style={{ width: "15px", height: "15px" }} />
						),
						value: "space-around",
					},
					{
						label: (
							<Icons.JustifyCenter style={{ width: "15px", height: "15px" }} />
						),
						value: "center",
					},
				],
			},
			{
				...
				options: [
					...
					{
						label: (
							<Icons.FAlignStart style={{ width: "15px", height: "15px" }} />
						),
						value: "start",
					},
					{
						label: (
							<Icons.FAlignEnd style={{ width: "15px", height: "15px" }} />
						),
						value: "end",
					},
					{
						label: (
							<Icons.FAlignStretch style={{ width: "15px", height: "15px" }} />
						),
						value: "stretch",
					},
					{
						label: (
							<Icons.FAlignCenter style={{ width: "15px", height: "15px" }} />
						),
						value: "center",
					},
				],
		...
		const categories = [
			...
			},
			{
				...
				children: (
					<CreateForm
						fields={
							transformKey == "rotate" ? extraFieldRotate : extraFieldScale
						}
						globalOnChange={this.globalOnChange}
						getFormData={(fn) => {
